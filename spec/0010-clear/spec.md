...
- **Installed Artifact**: an artifact recorded in the track file
  ...
  whose source registry is unregistered — clear removes everything in scope
  regardless of whether the source registry still exists.
...
- Clear differs from a registry teardown:
  [unset backend](../0012-backend/spec.md) performs a
  registry-scoped teardown that, by default, also removes the delivered
  artifacts (unless `--keep-artifacts`), whereas clear removes all installed
  artifacts in scope regardless of their source.
