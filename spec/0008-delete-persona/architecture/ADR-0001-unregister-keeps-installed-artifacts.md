...
Deleting a persona could mean one of two things: remove the definition so it stops shaping deliveries, or additionally cascade-remove every skill and agent the persona brought to the team's targets. A cascade removal is destructive — installed artifacts may be in active use, may also be claimed by other personas, or may have been modified locally. The same reasoning led `0005-delete-repository` to keep installed artifacts when a repository is deleted.

## Decision

Deletion **removes the persona definition only**. The entry is removed from `personas[]` in `~/.sauron/settings.yaml`; skills and agents already installed are left untouched (FR-003). Reconciling the environment with the new set of personas is the sync feature's job (`0011-sync`): its next run computes the new desired set and removes out-of-scope artifacts deliberately, as visible `-` lines in its plan.
