...
Repositories of kind `http` are HTTP(S) servers serving artifacts. This
capability defines the HTTP-specific behavior of
[add repository](../spec.md): URL validation, the `HEAD` reachability probe,
...
- **FR-001**: Sauron shall provide the ability to register an HTTP(S) server
  as a repository source of artifacts.
...
- **FR-007**: If the server cannot be reached (connection, TLS, or non-success
  ...
  configuration unchanged, and report that the repository cannot be reached.
