...
Defines the command-line interface for registering a repository of any kind
...
Command hierarchy: `sauron` (root) → `add` (group) → `repository`
...
$ sauron add repository --kind filesystem --priority 3 team-skills ./team-skills
Registered repository 'team-skills': /home/user/team-skills

# git over SSH (explicit kind required)
$ sauron add repository --kind git --priority 4 team-git git@github.com:acme/skills.git
Registered repository 'team-git': git@github.com:acme/skills.git

# Kind-scoped flag on the wrong kind (usage error, exit 2)
$ sauron add repository --kind filesystem --priority 5 --ssh-key ~/.ssh/id_ed25519 t ./t
Error: --ssh-key applies only to git repositories

# Non-SSH git URI rejected (usage error, exit 2)
$ sauron add repository --kind git --priority 6 web https://github.com/acme/skills.git
