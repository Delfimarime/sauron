...
An HTTP repository may sit behind HTTP Basic auth, so the user needs a way to supply a username and password. Persisting raw credentials in `~/.sauron/settings.yaml` would require secret management — encryption at rest (e.g. AES-GCM), a key, and a place to keep that key (key file or OS keychain). That machinery is non-trivial and easy to get subtly wrong.
