...
Every repository already carries a priority that is unique across all repositories regardless of kind (`0001`–`0003`), with a lower value meaning higher precedence — but until now that priority had no operational meaning.
...
Persona priority plays a different role: it does not select artifact contents (two personas naming the same artifact get the same resolved artifact); it only determines which persona is recorded as provenance when several bring the same artifact into the desired set (`0007-import-persona` ADR-0001).
