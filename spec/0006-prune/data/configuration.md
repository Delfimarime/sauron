...
Describes the data the Prune feature reads and updates. Prune compares installed artifacts (`track.json`) against registered repositories (`settings.json`) and removes the orphans.

## Inputs

### Registered repositories — `~/.sauron/settings.json`
...
### Tracking record — `~/.sauron/track.json`
...
- **Path**: `~/.sauron/track.json` (home directory resolved per platform).
...
- Prune writes only `track.json`, never `settings.json`.
- Updates are atomic: serialize to a temporary file in `~/.sauron/`, then rename over `track.json`. The file is left untouched on `--dry-run` or when nothing is orphaned.
...
{
  "installed": [
    { "type": "skill", "name": "code-review",   "target": "~/.claude/skills/code-review",   "repository": "team-deploy" },
    { "type": "agent", "name": "triager",       "target": "~/.claude/agents/triager.md",    "repository": "old-http" }
  ]
}
```
