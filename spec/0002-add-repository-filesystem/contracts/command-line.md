...
|------|----------|--------|-------------|
| `--kind` | Yes | `filesystem` | Selects the repository kind. `filesystem` must be given explicitly because `http` is the default when `--kind` is omitted. The `sauron add repository` interface is shared; other kinds are covered by their own features. Realizes FR-002, FR-010. |
...
Error: unknown repository kind 'something-else'
