...
required by [set registry FR-003](0001-set-registry/spec.md#ubiquitous) and the
[`--password` flag rule](0001-set-registry/contracts/set-registry.md#flags).
...
files reveal which registry and artifacts a developer uses. Owner:
...
[set registry command line — Flags](0001-set-registry/contracts/set-registry.md#flags);
the applicable set per transport is fixed by the
[git](0001-set-registry/capabilities/git.md) and
[http](0001-set-registry/capabilities/http.md) capabilities.
...
Every installed artifact lands at `sauron-<name>`; the `sauron-` prefix marks
ownership, so Sauron only ever touches artifacts it installed and never disturbs
anything else in a provider's directories. Owner:
