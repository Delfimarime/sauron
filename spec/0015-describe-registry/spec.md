...
registry in full — its kind, location, priority, and how it authenticates — so
...
resource, and never writes the settings or the track file.
...
- **FR-003**: When a user describes a registry, Sauron shall read the named
  registry from the settings and display its fields, one per line as
  `field: value`, with the identity field `name` first.
- **FR-004**: When displaying a registry's fields, Sauron shall present `name`
  as the identity and `kind`, `location`, `priority`, `auth`, `tls`, `timeout`,
  and `ssh-key` as the available fields, using the kind-appropriate locator
  (`path` for `filesystem`, `url` for `http`, `uri` for `git`) as `location`.
...
- **FR-010**: If the settings file does not exist, then Sauron shall treat the
...
- **FR-012**: If `--fields` names a field other than `name`, `kind`,
  `location`, `priority`, `auth`, `tls`, `timeout`, or `ssh-key`, then Sauron
...
- **Registry**: a registered source of artifacts (see
  ...
  `name` and the fields `kind`, `location`, `priority`, `auth`, `tls`,
  `timeout`, and `ssh-key`. The `location` is the kind's locator
  (`path`/`url`/`uri`). Fields that do not apply to the resolved kind are shown
  empty; `auth` shows only the `${env:VAR}` reference and never a resolved
  secret.
