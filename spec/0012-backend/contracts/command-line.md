...
`git` must be selected explicitly. The registry is a singleton — re-running
...
| `--keep-artifacts` | `unset backend` | all | No | false | — | Remove the configuration, catalog, and install records but leave the delivered artifacts in place. Realizes [spec](../spec.md) FR-010. |
...
- **`unset backend` success**: a report of what was torn down — the
  configuration, catalog, and install records; when artifacts are removed (no
  `--keep-artifacts`), they are listed in the shared plan/report format, grouped
  under `skills:` and `agents:` headings, one artifact per line prefixed `-`,
  followed by a summary count line. When nothing is configured, a line noting
  nothing was removed. Realizes [spec](../spec.md) FR-008, FR-009, FR-010, FR-012.
...
|------|---------|----------|
| `0` | Backend set; or torn down; or no registry was configured (idempotent teardown) | [spec](../spec.md) FR-005, FR-007, FR-008, FR-012 |
| `2` | Usage error — missing `<location>`, unknown `--kind`, a raw (non-env) credential, a malformed location for the kind, an invalid `--timeout`, a kind-scoped flag on a different kind | [spec](../spec.md) FR-013, FR-014, FR-015, FR-017, FR-019, FR-020; [http](../capabilities/http.md) FR-006, FR-008, FR-010; [git](../capabilities/git.md) FR-005, FR-007 |
| `1` | Runtime error — backend unreachable, an unset `${env:VAR}`, an unreadable settings/track file, or a failed artifact removal during teardown | [spec](../spec.md) FR-016, FR-018, FR-021; [http](../capabilities/http.md) FR-007; [filesystem](../capabilities/filesystem.md) FR-005; [git](../capabilities/git.md) FR-006 |
...
# Re-running overrides the previous registry (singleton upsert)
...
# Tear down everything (config + catalog + install records + artifacts)
$ sauron unset backend
skills:
- code-review
agents:
- software-engineer
Removed backend, catalog, and install records; cleared 2 artifacts (1 skill, 1 agent).

# Tear down but keep delivered artifacts
$ sauron unset backend --keep-artifacts
Removed backend, catalog, and install records; kept delivered artifacts.
