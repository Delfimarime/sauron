...
| `--username` | `set backend` | http | No | — | `${env:VAR}` only | Basic-auth user, env reference only. Realizes [http](../capabilities/http.md) FR-004, FR-010, FR-011. |
| `--password` | `set backend` | http | No | — | `${env:VAR}` only | Basic-auth password, env reference only. Realizes [http](../capabilities/http.md) FR-004, FR-010, FR-011. |
| `--skip-tls-verify` | `set backend` | http | No | false | — | Skip server cert verification. Realizes [http](../capabilities/http.md) FR-003. |
| `--ca-cert` | `set backend` | http | No | — | path | CA bundle to trust the server. Realizes [http](../capabilities/http.md) FR-003, FR-012. |
| `--client-cert` | `set backend` | http | No | — | path | Client cert for mutual TLS; requires `--client-key`. Realizes [http](../capabilities/http.md) FR-003, FR-009, FR-012. |
| `--client-key` | `set backend` | http | No | — | path | Client key for mutual TLS; requires `--client-cert`. Realizes [http](../capabilities/http.md) FR-003, FR-009, FR-012. |
| `--ssh-key` | `set backend` | git | No | system SSH | path | Private key to authenticate; omitted = system SSH credentials (agent, `~/.ssh/config`, default keys). Realizes [git](../capabilities/git.md) FR-008, FR-010. |
| `--timeout` | `set backend` | http, git | No | `30s` | duration | Bounds network operations: the HTTP `HEAD` probe, `git ls-remote`. Realizes [http](../capabilities/http.md) FR-005, FR-013 and [git](../capabilities/git.md) FR-004, FR-009. |
...
| `2` | Usage error — missing `<uri>`, unknown `--kind`, an invalid `--timeout`, a kind-scoped flag on a different kind; **http**: invalid (non-`http`/`https`) URL, raw (non-env) credential, unpaired `--client-cert`/`--client-key`; **git**: non-SSH or malformed git URI | [spec](../spec.md) FR-013, FR-014, FR-015, FR-017, FR-019, FR-020; [http](../capabilities/http.md) FR-007, FR-009, FR-010, FR-013; [git](../capabilities/git.md) FR-006, FR-009 |
| `1` | Runtime error — an unreadable `backend.yaml`/`personas.yaml`/`track.yaml`, or a failed artifact removal during teardown; **http**: server unreachable (connection, TLS, or non-success `HEAD`), unset `${env:VAR}`, unreadable CA/cert/key file; **filesystem**: directory not accessible; **git**: remote unreachable or authentication failed, unreadable `--ssh-key` file | [spec](../spec.md) FR-016, FR-018, FR-021; [http](../capabilities/http.md) FR-008, FR-011, FR-012; [filesystem](../capabilities/filesystem.md) FR-005; [git](../capabilities/git.md) FR-007, FR-008 |
...
# git over SSH with an explicit key
$ sauron set backend --kind git --ssh-key ~/.ssh/personas_ed25519 git@github.com:acme/personas.git
Configured backend: git@github.com:acme/personas.git

# http with mutual TLS
$ sauron set backend \
    --client-cert ~/.sauron/client.pem --client-key ~/.sauron/client.key \
    https://secure-personas.example.com
Configured backend: https://secure-personas.example.com

# Unpaired client cert (usage error, exit 2)
$ sauron set backend --client-cert ~/.sauron/client.pem https://personas.example.com
Error: --client-cert and --client-key must be provided together for mutual TLS
