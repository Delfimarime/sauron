...
singleton backend from the settings and contacts no external resource. The
command takes no name, because there is exactly one backend per instance.
...
|------|----------|---------|--------|-------------|
| `--fields` | No | all fields | comma-separated `location`, `kind`, `auth`, `catalog-size`, `installed`, `last-synced` | Fields to display, in order; `location` is always present and first. Realizes [spec](../spec.md) FR-013, FR-010. |

## Output

- **Success (configured)**: the backend's fields on stdout, one per line as
  `field: value`, with `location` first. Fields that have no value are shown with
  an empty value. `auth` is rendered `REDACTED`; counts (`catalog-size`,
  `installed`) are printed as integers. Realizes [spec](../spec.md) FR-003,
...
| `1` | Runtime error — the settings cannot be read or parsed | [spec](../spec.md) FR-011 |

## Examples

```
# Describe the configured backend (auth always REDACTED, counts as integers)
...
# A subset of fields, in the given order (location always first)
$ sauron describe backend --fields kind,catalog-size
location: https://secure-personas.example.com
kind: http
catalog-size: 2
...
Error: --fields must name only: location, kind, auth, catalog-size, installed, last-synced
```
