...
Scheduling installs a **managed entry in the user's crontab** that runs `sauron sync`. The entry is bracketed by a `# managed by sauron` marker so Sauron only ever reads, replaces, or removes its own line and never disturbs other crontab content:
...
- The scheduled command is plain `sauron sync`, so it always follows the current global target and persona configuration.
...
A native scheduler integration (`launchd` agent, systemd timer) is wanted, or scheduling needs to run more than the single `sauron sync` job.
