...
// TestRegistriesStoreListReturnsAll decodes every stored registry.
func TestRegistriesStoreListReturnsAll(t *testing.T) {
	// Arrange: two registries persisted through the typed store.
	...
	require.NoError(t, registries.Add(context.Background(), types.Registry{
		...
		Spec:     types.RegistrySpec{Transport: types.TransportHTTP, URI: "https://example.com/beta"},
	}))

	// Act.
	got, err := registries.List(context.Background())

	// Assert: both registries are returned, decoded.
	require.NoError(t, err)
	require.Len(t, got, 2)
	assert.Equal(t, acmeName, got[0].Metadata.Name)
	assert.Equal(t, "beta", got[1].Metadata.Name)
}

// TestRegistriesStoreListEmpty returns an empty slice when nothing is stored.
func TestRegistriesStoreListEmpty(t *testing.T) {
	// Arrange.
	registries, _ := newTestRegistriesStore(t)
...
// TestRegistriesStoreListPropagatesReadError surfaces a read failure from the store.
func TestRegistriesStoreListPropagatesReadError(t *testing.T) {
	// Arrange: a malformed stream makes the underlying FindAll fail.
	...
	// Act.
	_, err := registries.List(context.Background())
