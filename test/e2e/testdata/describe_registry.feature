...
Feature: Describe registry
  ...
  Scenario: shows the full detail of the configured registry
    Given the registry is configured:
      | transport | source                            | revision | timeout |
    ...
    And the descriptor shows source as git@github.com:acme/artifacts.git
    And the descriptor shows revision as v1.2.0
    And the descriptor shows timeout as 45s

  # FR-003 — --fields selects and orders the displayed fields, source first.
  Scenario: shows only the selected fields with --fields
    Given the registry is configured:
      | transport | source                            |
      | git       | git@github.com:acme/artifacts.git |
    When the user runs sauron describe registry --fields transport,source
    Then the command succeeds
    And the descriptor shows source as git@github.com:acme/artifacts.git
    And the descriptor shows transport as git
    And the output does not contain revision
  ...
  Scenario: shows the auth block as environment references, never a secret
    Given the registry is configured:
      | transport | source                            | username         | password          |
      | git       | git@github.com:acme/artifacts.git | ${env:ACME_USER} | ${env:ACME_TOKEN} |
    When the user runs sauron describe registry
    Then the command succeeds
    And the output contains credentials:
  ...
  # verbatim: source first (no name line), the aligned label: value column, and the
  ...
  Scenario: renders the full descriptor structure with timestamps, tls, and sshKey
    ...
    When the user runs sauron describe registry
    Then the command succeeds
    And the descriptor shows created as 2026-06-21T07:30:00Z
    And the descriptor shows updated as 2026-06-22T08:00:00Z
