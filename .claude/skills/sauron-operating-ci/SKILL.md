...
1. **Parity, not duplication.** Each CI job runs the **identically-named Taskfile
   ...
   Never reimplement gate logic in the pipeline — call the task.
2. **Stage gating.**
   - Stage 1 (parallel): `test`, `gate-lint`.
   - Stage 2 (parallel, on stage 1 success): `build`, `gate-coverage`.
   - Stage 3 (on stage 2 success): `gate-security`.
   - Stage 4 (on stage 3 success, **default branch only**): `publish`.
3. **Artifact hand-off.** `test` publishes the coverage report consumed by
   `gate-coverage`; `build` publishes the binary consumed by `gate-security` and
   `publish`.
...
5. **Publish.** Only on the default branch: generate the binary's SHA-256
   checksum and publish the binary + `.sha256` as **Release assets** (GitHub
   Release / GitLab Release). No OCI artifacts, no package registry.
